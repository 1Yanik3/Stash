<script lang="ts">
  ...
  import { UploadImportSource, type ImportSource } from "./ImportSources.svelte"
  ...
  {#snippet sidebar()}
    ...
    >
      <div class="sidebar-header">
        <Button card icon="mdiUpload" onclick={() => fileInputElement.click()}
          >Upload</Button
        >
        <Button card icon="mdiImport" disabled>Import</Button>
        <Button card icon="mdiTransmissionTower" disabled>Transfer</Button>
      </div>
        ...
        {#each queue as queueEntry}
          ...
          >
            ...
            <Button
              noMargin
              icon="mdiClose"
              onclick={() => queue.splice(queue.indexOf(queueEntry), 1)}
